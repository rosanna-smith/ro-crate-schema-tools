...
class SOSS {
  ...
  async setup() {
    ...
    const schemaJson  = await jsonld.compact(schemaJsonRaw , sacrificialCrate["@context"]);

    for (let entity of schemaJson["@graph"]) {
      //entity["@id"] = entity["@id"].replace(/^schema:/, "http://schema.org/");
      
      this.backgroundSchema.addValues(this.backgroundSchema.rootDataset, "mentions", entity);
    }
    ...
    for (let entity of  this.backgroundSchema.getGraph())
    {
      if (entity["@id"].match("^schema:")) {
        this.backgroundSchema.updateEntityId(entity, entity["@id"].replace(/^schema:/, "http://schema.org/") )
      }
    }
  }
