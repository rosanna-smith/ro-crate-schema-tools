...
const { clone, union, isArray } = require("lodash");
...
class SOSS {
   ...
   */
  constructor(soss = new ROCrate({ link: true, array: true }), ns = "http://example.com/todo#", config = {}) {
    this.backgroundSchema = new ROCrate({ array: true, link: true });
    this.ns = new Promise((resolve, reject) => {
  ...
  async setup() {
    ...
    const rocJson = await fetch(profileCrate);
            ...
            if (val["@type"]) {
              return val["@type"].map((t) => {
                ...
                if (
                  ...
                  this.sossCrate.addEntity(newTerm);
                  if (this.confif.addSuperClasses) this.addSuperclasses(newTerm);
